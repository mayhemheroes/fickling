#!/usr/bin/env bash
#
# mayhem/test.sh — RUN fickling's own FULL upstream test suite (test/, unittest TestCases
# collected by pytest; deps — pytest, torch, torchvision, numpy, py7zr — already installed by
# mayhem/build.sh) and emit a CTRF (ctrf.io) summary. exit 0 iff failed==0. PATCH-grade oracle:
# a no-op patch that neuters the library breaks the suite's behavioral assertions (asserted
# severities / decompiled ASTs / golden outputs), so it FAILS here (anti-reward-hacking).
#
# It does NOT compile — build.sh installed the deps into the in-image site dir and compiled the
# fickling_run_tests ELF wrapper. We only RUN the suite, routed through that compiled NON-system
...
# Put the in-image site dir (pytest + torch + ...) and the fickling source tree on PYTHONPATH.
...
# -o addopts= drops any coverage addopts so the run doesn't need writable coverage output.
LOG="$(mktemp)"
